...
def _prepare_dashboard_app(
    ...
    vector_store_stub = types.ModuleType("job_search_tool.vector_store")
    vector_store_stub.get_vector_store = lambda *args, **kwargs: None
    monkeypatch.setitem(sys.modules, "job_search_tool.vector_store", vector_store_stub)
    monkeypatch.setenv("JOB_SEARCH_DATA_DIR", str(tmp_path))
    monkeypatch.delenv("JOB_SEARCH_CONFIG", raising=False)
    sys.modules.pop("job_search_tool.config", None)
    sys.modules.pop("job_search_tool.dashboard", None)
    st.cache_data.clear()
    st.cache_resource.clear()

    app = AppTest.from_file(
        str(Path(__file__).parent.parent / "src" / "job_search_tool" / "dashboard.py"),
        default_timeout=10,
    )
    return app, db_dir
...
def test_filtered_jobs_csv_bytes_sanitizes_formula_payloads(monkeypatch) -> None:
    ...
    from job_search_tool.dashboard import _filtered_jobs_csv_bytes
...
class TestEscapeHtmlText:
    ...
    def test_normal_string(self) -> None:
        from job_search_tool.dashboard import _escape_html_text
...
class TestScoreBadgeHtml:
    ...
    def test_high_score(self) -> None:
        from job_search_tool.dashboard import SCORE_HIGH, _score_badge_html
    ...
    def test_medium_score(self) -> None:
        from job_search_tool.dashboard import SCORE_MED, _score_badge_html
    ...
    def test_low_score(self) -> None:
        from job_search_tool.dashboard import _score_badge_html
...
class TestFormatSalary:
    ...
    def test_range_usd(self) -> None:
        from job_search_tool.dashboard import _format_salary
...
class TestShortNum:
    ...
    def test_millions(self) -> None:
        from job_search_tool.dashboard import _short_num
...
class TestDaysAgo:
    ...
    def test_today(self) -> None:
        from job_search_tool.dashboard import _days_ago
